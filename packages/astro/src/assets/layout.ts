...
export const getWidths = ({
	width,
	layout,
	screenBreakpoints = DEFAULT_RESOLUTIONS,
	originalWidth,
}: {
	width?: number;
	layout: ImageLayout;
	screenBreakpoints?: Array<number>;
	originalWidth?: number;
}): Array<number> => {
  if (layout === 'none') return []

  const screenBreakpointsSorted = screenBreakpoints.sort((a, b) => a - b);

	const smallerThanOriginal = (w: number) => !originalWidth || w <= originalWidth;

	// For full-width layout we return all breakpoints smaller than the original image width
	if (layout === 'full-width') {
		return screenBreakpointsSorted.filter(smallerThanOriginal);
	}
  ...
  const largestScreenSize = Math.min(screenBreakpointsSorted.at(-1)! * 2, 6016)
